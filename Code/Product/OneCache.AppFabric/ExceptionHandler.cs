...
namespace OneCache.AppFabric
{
	...
	{
		private const int MillisecondsRetry = 100;
		...
		public HandleExceptionResult Handle(DataCacheExceptionWrapper exception)
		{
			...
			switch (exception.ErrorCode)
			{
				...
				case DataCacheErrorCode.Timeout:
					//AppFabric raises a timeout in some cases like when using a non alphanumeric region
				...
				case DataCacheErrorCode.RetryLater:
					...
					return new HandleExceptionResult(false, true);
				default:
					//those that are not handled will need to have a custom compensation mechanism, see CreateRegion() as is the only one implemented here
					Logger.ErrorFormat("Unhandled AppFabric Exception. This could suggest to implement a new concrete handler: {0}",
						exception);
					break;
			}
			return new HandleExceptionResult(false, true);
